...
builder.Services.AddValidatorsFromAssembly(Assembly.Load("Jassi.Validators"));
#endregion
...
builder.Services.AddSwaggerGen(c =>
{
    ...
    {
        Version = "v1",
        Title = "Apis for Jassi.me",
        Description = "Apis for Jassi.in"
    });
});
...
var jassiConfigSection = builder.Configuration.GetSection("JassiConfig");
var jassiConfig = builder.Configuration.GetSection("JassiConfig").Get<AlmondcoveConfig>();

builder.Services.Configure<AlmondcoveConfig>(jassiConfigSection);

builder.Services.AddScoped<IDataService>(provider =>
{
    return new DataService(jassiConfig.ConnectionString.ToString());
});
